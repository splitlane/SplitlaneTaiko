...
function Taiko.PlaySong(Parsed, Difficulty)
    ...
    local statusflicker = 50 --Status flicker (Delay from startms render) (good/ok/bad) (ms)
    ...
    local function RenderStatus(out, status)
        local t = timingpixel[status.status]
        local o = t.Offset
        local c = t.Color.All --Color for all
        local ox, oy = 0, -math.floor(noteradius * 1.6) - 8
